...
from pages.models import Page, Domain, PageStar #TODO: Unify PageStar and PostStar into a singular Star model
...
class ShareObjectTests(TestCase):

    def setUp(self):
        self.stella = User.objects.create_user(username='stella')
        self.stella.set_password('pass')
        self.stella.save()
        self.contact = User.objects.create_user(username='contact')
        self.contact.set_password('pass')
        self.contact.save()
...
class NotificatoinSignalTests(TestCase):
    ...
    def test_post_starred_notifies_author(self):
        PostStar.objects.create(user=self.stranger, post=self.post)
        self.assertTrue(Notification.objects.filter(
    ...
    def test_page_also_starred_notifies_contact(self):
        ...
        PageStar.objects.create(user=self.stella, page=self.page)
        PageStar.objects.create(user=self.stranger, page=self.page)
        self.assertTrue(Notification.objects.filter(
            recipient=self.stella,
    ...
    def test_post_also_starred_notifies_contact(self):
        ...
        PostStar.objects.create(user=self.stella, post=self.post)
        PostStar.objects.create(user=self.stranger, post=self.post)
        self.assertTrue(Notification.objects.filter(
            recipient=self.stella,
